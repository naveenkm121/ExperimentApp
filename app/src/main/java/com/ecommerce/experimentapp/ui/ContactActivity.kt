        ...
        call.enqueue(object : Callback<ContactReq> {
            override fun onResponse(call: Call<ContactReq>, response: Response<ContactReq>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(baseContext, "get Contacts list successfully from server", Toast.LENGTH_LONG).show()
                } else {
                    DebugHandler.log( "Upload failed: ${response.errorBody().toString()}")
                    Toast.makeText(baseContext, "Failed to get  Contacts from server: ${response.errorBody().toString()}", Toast.LENGTH_LONG).show()
                }
            }
